    ...
    def __init__(self, sw_version, signer_pubkey_filename,
        ...
        for line in image_list:
            if not re.match(r'^\s*#|^\s*$', line):   # ignore commented out and empty lines
                ...
                # Check the max image size is greater than the actual image size
                if image_max_size < os.path.getsize(image_filename):
                    raise ContentCertCreationError("Invalid max image size in images-table: max image size must be greater than the actual image size")
