...
/*
 * Apollo510 (Cortex-M55) DCACHE line size.
 *
 * The user context aliases ChachaContext_t (mbedtls_chacha_init checks that
 * their sizes are equal), so it must carry the same alignment. Both pad up to
 * the next multiple of CC_DCACHE_LINE_BYTES, keeping their sizes equal, so the
 * CC312 symmetric DMA never shares a cache line between the context and an
 * unrelated allocation.
 */
#ifndef CC_DCACHE_LINE_BYTES
#define CC_DCACHE_LINE_BYTES  32U
#endif
...
}__attribute__((aligned(CC_DCACHE_LINE_BYTES))) mbedtls_chacha_user_context;
