 ...
 *
 * Any DLLI buffer / context that the CC312 symmetric DMA touches is aligned to
 * this so DCACHE clean/invalidate maintenance (see chacha_driver.c) operates on
 * whole cache lines and never disturbs neighboring data.
 */
...
} __attribute__((aligned(CC_DCACHE_LINE_BYTES))) ChachaContext_t;
