...
// #### INTERNAL END ####
    ...
    ui32Fsel++;
    // #### INTERNAL BEGIN ####
    // [FBRDSW-329] Apollo510L input clock frequency can't be that high.
    // #### INTERNAL END ####
    if (ui32Fsel < 3)
    {
        //
        // This is an error, can't go that high.
        return 0;
    }
    ...
    ui32ClkCfg = _VAL2FLD(IOM0_CLKCFG_FSEL,   ui32Fsel - 3) |
...
am_hal_iom_enable(void *pHandle)
{
    ...
    if ( pIOMState->pNBTxnBuf )
    {
        ...
        //
// #### INTERNAL BEGIN ####
// Do not set CMDCMP trigger. Hardware already handles this internally for the
// case where there is one DMA per command.
// Enabling this trigger could cause other issues - CORVETTE-877
// TODO - Evaluate with Apollo4 FPGA before reverting to expected behavior.
#if 0
        IOMn(pIOMState->ui32Module)->DMATRIGEN =
                                    _VAL2FLD(IOM_DMATRIGEN_DTHREN, 1)   |
                                    _VAL2FLD(IOM_DMATRIGEN_DCMDCMPEN, 1);
#else
// #### INTERNAL END ####
...
am_hal_iom_status_get(void *pHandle, am_hal_iom_status_t *psStatus)
{
    ...
    //
// #### INTERNAL BEGIN ####
// (TODO: This should be changed to use individual members).
// #### INTERNAL END ####
...
{
    uint64_t ui64ClkActualFreq_CLkcfg;
    uint32_t ui32ClkCfg;
    am_hal_iom_state_t *pIOMState = (am_hal_iom_state_t*)pHandle;
    uint32_t status = AM_HAL_STATUS_SUCCESS;
    uint32_t ui32Module;
    ...
    if ( psConfig->eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
        ...
        //
        ui64ClkActualFreq_CLkcfg = iom_get_interface_clock_cfg(psConfig->ui32ClockFreq, (psConfig->eSpiMode & 2) >> 1);
        if (ui64ClkActualFreq_CLkcfg == 0)
        {
            return AM_HAL_STATUS_INVALID_ARG;
        }
        ui32ClkCfg = (uint32_t)ui64ClkActualFreq_CLkcfg;
        //
...
uint32_t am_hal_iom_control(void *pHandle, am_hal_iom_request_e eReq, void *pArgs)
{
    ...
    switch (eReq)
    {
...
#endif // AM_HAL_DISABLE_API_VALIDATION
                IOMn(ui32Module)->CQCFG_b.CM4MSPIFLGSEL = *((uint32_t *)pArgs);
            }
            else
            {
                status = AM_HAL_STATUS_INVALID_ARG;
        ...
        case AM_HAL_IOM_REQ_LINK_CM4:
            {
                IOMn(ui32Module)->CQCFG_b.CM4MSPIFLGSEL = IOM0_CQCFG_CM4MSPIFLGSEL_CM4FLGSEL;
            }
            break;
...
// #### INTERNAL BEGIN ####
        case AM_HAL_IOM_REQ_LINK_GPIO:
#ifndef AM_HAL_DISABLE_API_VALIDATION
            if (!pArgs)
            {
                return AM_HAL_STATUS_INVALID_ARG;
            }
            if ( *((uint32_t *)pArgs) >= AM_HAL_GPIO_MAX_PADS )
            {
                return AM_HAL_STATUS_INVALID_ARG;
            }
#endif // AM_HAL_DISABLE_API_VALIDATION
            switch (pIOMState->ui32Module)
            {
                case 0:
                    GPIO->IOM0IRQ_b.IOM0IRQ = *((uint32_t *)pArgs);
                    break;
                case 1:
                    GPIO->IOM1IRQ_b.IOM1IRQ = *((uint32_t *)pArgs);
                    break;
                case 2:
                    GPIO->IOM2IRQ_b.IOM2IRQ = *((uint32_t *)pArgs);
                    break;
                case 3:
                    GPIO->IOM3IRQ_b.IOM3IRQ = *((uint32_t *)pArgs);
                    break;
                case 4:
                    GPIO->IOM4IRQ_b.IOM4IRQ = *((uint32_t *)pArgs);
                    break;
                case 5:
                    GPIO->IOM5IRQ_b.IOM5IRQ = *((uint32_t *)pArgs);
                    break;
                default:
                    status = AM_HAL_STATUS_INVALID_ARG;
            }
